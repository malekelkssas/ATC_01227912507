...
import { TranslationConstants, UPLOAD_IMAGES_CONSTANTS, EventFields } from "@/utils/constants";
import { UpdateEventZod, type UpdateEventDto } from "@/types/dtos";
...
import type { GetEventAdminResponseDto, GetFullTagsResponseDto } from "@/types/dtos";
import {
    Command,
    CommandEmpty,
    CommandGroup,
...
interface UpdateEventDialogProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    event: GetEventAdminResponseDto;
}

const UpdateEventDialog = ({ isOpen, onClose, onSuccess, event }: UpdateEventDialogProps) => {
    ...
    const {
        ...
        defaultValues: {
            ...
            [EventFields.CATEGORY]: event.category.map((category) => category._id),
            [EventFields.PRICE]: event.price,
            [EventFields.DATE]: new Date(event.date),
        }
    });
    ...
    const onSubmit = async (data: UpdateEventDto) => {
        console.log({
            ...data,
                [EventFields.DATE]: new Date(data[EventFields.DATE] || new Date()),
        });
        setIsLoading(true);
        try {
            ...
            formData.append(UPLOAD_IMAGES_CONSTANTS.DATA_FIELD_NAME, JSON.stringify({
                ...data,
                [EventFields.DATE]: new Date(data[EventFields.DATE] || new Date()),
            }));

            await EventService.patchEvent(event._id, formData);
            toast({
            ...
            <DialogContent className="sm:max-w-[600px] dark:bg-card border-duck-yellow/20">
                <DialogHeader>
                    <DialogTitle className="text-foreground rtl:text-right ltr:text-left rtl:pr-10">
                        {t(TranslationConstants.ADMIN.EVENTS.UPDATE_EVENT)}
                    </DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <DatePicker
                                selected={
                                    typeof watch(EventFields.DATE) === "string"
                                        ? new Date(watch(EventFields.DATE) || new Date())
                                        : watch(EventFields.DATE) || new Date()
                                }
                                    ...
                                    {isLoadingTags ? (
                                        ...
                                        <>
                                            {(selectedCategories?.length ?? 0) > 0
                                                ? `${selectedCategories?.length ?? 0} ${t(TranslationConstants.EVENTS.SELECTED)}`
