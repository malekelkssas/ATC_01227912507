...
import { UpdateTagZod, type UpdateTagDto, type GetFullTagsResponseDto } from "@/types/dtos";
...
interface UpdateTagDialogProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    tag: GetFullTagsResponseDto[0];
}

const UpdateTagDialog = ({ isOpen, onClose, onSuccess, tag }: UpdateTagDialogProps) => {
    ...
    const {
        ...
        resolver: zodResolver(UpdateTagZod),
        defaultValues: {
            [TagFields.NAME]: tag.name,
            [TagFields.COLOR]: tag.color,
        }
    });
    ...
    const onSubmit = async (data: UpdateTagDto) => {
        setIsLoading(true);
        try {
            await TagService.updateTag(tag._id, data);
            toast({
            ...
            <DialogContent className="sm:max-w-[425px] dark:bg-card border-duck-yellow/20">
                <DialogHeader>
                    <DialogTitle className="text-foreground rtl:text-right ltr:text-left rtl:pr-10">
                        {t(TranslationConstants.ADMIN.TAGS.UPDATE_TAG)}
                    </DialogTitle>
                </DialogHeader>
                    ...
                    <DialogFooter>
                        ...
                        >
                            {isLoading ? (
                                ...
                                t(TranslationConstants.COMMON.BUTTONS.UPDATE)
                            )}
                        </Button>
                    </DialogFooter>
                </form>
